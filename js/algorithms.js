...
class BufferPool {
  ...
  get(width, height, p) { const key = `${width}x${height}`; this.lastUsed.set(key, Date.now()); if (!this.buffers.has(key)) { const buffer = p.createGraphics(width, height); buffer.elt.getContext('2d', { willReadFrequently: true, alpha: false }); buffer.pixelDensity(1); this.buffers.set(key, buffer); } return this.buffers.get(key); }
  cleanup(maxAge = 60000) { const now = Date.now(); for (const [key, time] of this.lastUsed) { if (now - time > maxAge) { this.buffers.get(key)?.remove(); this.buffers.delete(key); this.lastUsed.delete(key); } } }
...
class LumaLUT {
  ...
  build(p5colors, p) { const count = p5colors.length; this.lut = new Uint8Array(256 * 3); for (let i = 0; i < 256; i++) { const index = count === 0 ? 0 : Math.min(Math.floor(i / 255 * count), count - 1); const color = p5colors[index] || p.color(i); this.lut[i * 3] = p.red(color); this.lut[i * 3 + 1] = p.green(color); this.lut[i * 3 + 2] = p.blue(color); } this.cachedColors = p5colors; }
  map(luma) { const index = Math.min(Math.max(Math.floor(luma), 0), 255); return [ this.lut[index * 3], this.lut[index * 3 + 1], this.lut[index * 3 + 2] ]; }
  needsRebuild(p5colors) { if (!this.cachedColors || this.cachedColors.length !== p5colors.length) return true; return false; }
}

class BayerLUT {
  constructor() { const BAYER_4x4 = [[0,8,2,10],[12,4,14,6],[3,11,1,9],[15,7,13,5]]; this.matrix = new Float32Array(16); for (let i = 0; i < 16; i++) { const y = Math.floor(i / 4); const x = i % 4; this.matrix[i] = (BAYER_4x4[y][x] / 16.0 - 0.5); } }
  get(x, y) { const index = (y % 4) * 4 + (x % 4); return this.matrix[index]; }
}

class BlueNoiseLUT {
  constructor() { this.noise = new Float32Array([ 0.53, 0.18, 0.71, 0.41, 0.94, 0.24, 0.82, 0.47, 0.12, 0.65, 0.29, 0.88, 0.06, 0.59, 0.35, 0.76, 0.76, 0.35, 0.94, 0.18, 0.71, 0.12, 0.88, 0.24, 0.24, 0.82, 0.47, 0.65, 0.29, 0.94, 0.41, 0.59, 0.88, 0.06, 0.71, 0.35, 0.82, 0.18, 0.65, 0.12, 0.41, 0.59, 0.12, 0.76, 0.24, 0.47, 0.94, 0.29, 0.65, 0.29, 0.88, 0.06, 0.59, 0.71, 0.35, 0.82, 0.18, 0.94, 0.24, 0.53, 0.12, 0.76, 0.47, 0.41 ]); }
  get(x, y) { const index = (y % 8) * 8 + (x % 8); return this.noise[index] - 0.5; }
}
...
function drawDither(p, buffer, src, w, h, cfg, lumaLUT, bayerLUT) {
  ...
  if (cfg.useOriginalColor) {
    // ... (Lógica para color original sin cambios)
  } else {
    if (cfg.effect === 'bayer') {
      // ... (Lógica para Bayer sin cambios)
    } else {
...
function drawVariableError(p, buffer, src, w, h, cfg, lumaLUT) {
  // ... (Sin cambios, ya que este algoritmo tiene su propia lógica adaptativa)
  const scale = cfg.ditherScale;
  const pw = Math.floor(w / scale), ph = Math.floor(h / scale);
  buffer.image(src, 0, 0, pw, ph);
  buffer.loadPixels();
